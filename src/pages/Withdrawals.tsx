...
export default function Withdrawals() {
  ...
  const { data: pendingCommissions } = useQuery({
    ...
    queryFn: async () => {
      ...
      const commissionsByBarber = transactionItems.reduce(
        (acc: Record<string, { name: string; total: number }>, item) => {
          if (item.barber_id) {
  ...
  const withdrawalsByBarber =
    withdrawals?.reduce((acc: Record<string, number>, w) => {
      if (w.barber_id) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola penarikan komisi/gaji barber
          </p>
        </div>
